...
export async function registerService(
  ...
  if (!username || !password) {
    console.log(
      "[REGISTER] Failed: missing_username_or_password"
    );
  ...
  if (username.length < 3) {
    console.log(
      "[REGISTER] Failed: username_too_short"
    );
  ...
  if (password.length < 6) {
    console.log(
      "[REGISTER] Failed: password_too_short"
    );
  ...
  if (exists) {
    console.log(
      "[REGISTER] Failed: username_already_exists"
    );
...
export async function loginService(
  ...
  if (!username || !password) {
    console.log(
      "[LOGIN] Failed: missing_username_or_password"
    );
  ...
  */
  ...
  if (!user.userId) {
    console.log(
      "[LOGIN] Failed: user_missing_userId",
      {
        _id: String(user._id),
        username: user.username,
      }
...
export async function resumeService(
  ...
  if (!token) {
    console.log(
      "[AUTH RESUME] Failed: missing_session_token"
    );
  ...
  if (!user) {
    console.log(
      "[AUTH RESUME] Failed: invalid_or_expired_session"
    );
