...
import {
  scheduleTweetNotificationExpiry,
...
/*
  أنواع إشعارات التويتات.
*/
export const TWEET_NOTIFICATION_TYPES = [
  "tweet_like",
  "tweet_comment",
  "tweet_retweet",
  "tweet_mention",
...
export type TweetNotificationType =
  (typeof TWEET_NOTIFICATION_TYPES)[number];
...
function isTweetNotificationType(
  value: string
): value is TweetNotificationType {
  return (
    TWEET_NOTIFICATION_TYPES as readonly string[]
  ).includes(value);
}
...
/*
  التحقق هل الحقل موجود داخل NotificationSchema.
*/
function hasNotificationPath(
  path: string
) {
  try {
    return Boolean(
      NotificationModel?.schema?.path(
        path
      )
    );
  } catch {
...
*/
async function buildSenderQuery(
  senderUserId: string
): Promise<Record<string, any>> {
  if (
    hasNotificationPath(
...
function buildTypeQuery(
  type:
    | TweetNotificationType
    | {
        $in:
          readonly TweetNotificationType[];
      }
): Record<string, any> {
  if (
    hasNotificationPath(
...
*/
async function resolveSenderPhotoUrl(
  senderUserId: string,
  ...
  try {
    ...
    console.error(
      "[TWEET NOTIFICATION SENDER PHOTO ERROR]",
      {
        senderUserId,
        error,
      }
...
*/
async function safelyScheduleNotificationExpiry(
  ...
  try {
    ...
    */
    ...
    await scheduleTweetNotificationExpiry(
      notificationId
    );

    console.log(
      "[TWEET NOTIFICATION EXPIRY SCHEDULED]",
      {
        notificationId,
      }
    );
    ...
    */
    console.error(
      "[TWEET NOTIFICATION EXPIRY ERROR]",
      {
        notificationId,
        error,
      }
...
export async function createTweetNotification(
  ...
  */
  ...
  if (
    !isTweetNotificationType(
      type
    )
  ) {
    console.error(
      "[TWEET NOTIFICATION] Invalid type:",
      {
        type,
      }
    );
  ...
  const senderPhotoUrl =
    await resolveSenderPhotoUrl(
      senderUserId,
      providedSenderPhotoUrl
    );
  ...
  try {
    ...
    */
    await safelyScheduleNotificationExpiry(
      notification.notificationId
    );
...
console.log(
  "[TWEET NOTIFICATION SOCKET SEND]",
  {
    recipientUserId,
    payload:
      socketPayload,
...
console.log(
  "[TWEET NOTIFICATION SOCKET RESULT]",
  {
    recipientUserId,
    sent,
  }
...
*/
async function buildReactionNotificationQuery(
  input: {
    recipientUserId: string;
    senderUserId: string;
    tweetId: string;
  ...
  const recipientQuery =
    await buildRecipientQuery(
      input.recipientUserId
    );

  const senderQuery =
    await buildSenderQuery(
      input.senderUserId
    );
...
*/
export async function deleteTweetLikeNotification(
  input: {
    recipientUserId: string;
    senderUserId: string;
    tweetId: string;
  ...
  if (
    ...
    return {
      ...
      reason:
        "invalid_notification_delete_input",
    };
  }

  const query =
    await buildReactionNotificationQuery({
      recipientUserId,
  ...
  if (
    ...
    try {
      ...
      console.error(
        "[DELETE LIKE NOTIFICATION REDIS ERROR]",
        {
          notificationId,
          error,
        }
...
*/
export async function deleteTweetRetweetNotification(
  input: {
    recipientUserId: string;
    senderUserId: string;
    tweetId: string;
  ...
  if (
    ...
    try {
      ...
      console.error(
        "[DELETE RETWEET NOTIFICATION REDIS ERROR]",
        {
          notificationId,
          error,
        }
...
*/
export async function deleteAllTweetNotifications(
  tweetIdValue: string
) {
  const tweetId =
    cleanValue(
  ...
  for (
    ...
    try {
      ...
      console.error(
        "[DELETE ALL TWEET NOTIFICATIONS REDIS ERROR]",
        {
          notificationId,
